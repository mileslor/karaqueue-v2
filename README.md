...
### 同步歌詞
- 播放歌曲時自動從網易雲音樂搜尋對應 LRC 時間軸歌詞
- 繁體中文顯示，支援個別歌曲時間偏移及全體偏移調節
- 可手動編輯或清除歌詞，設定儲存於歌曲庫供下次重用
- 桌面控制台（`/`）及手機搖控（`/remote`）均可控制字幕開關
...
- **歌曲庫**：JSON 檔案，AI 自動分類
- **歌詞**：網易雲音樂非官方 API（伺服器代理請求）+ 歌詞快取（lyrics-cache.json）
...
- **YouTube 內容**：所有視頻透過 YouTube IFrame Player API 播放，版權歸原作者及 YouTube 所有，使用須遵守 [YouTube 服務條款](https://www.youtube.com/t/terms)。
- **歌詞內容**：歌詞由網易雲音樂非官方 API 自動獲取，版權歸原作者及網易雲音樂所有。本系統不儲存或分發受版權保護之歌詞作任何商業用途，快取僅用於改善個人使用體驗。
- 使用者須自行確保使用符合所在地區法律法規。開發者對任何版權糾紛不承擔責任。
